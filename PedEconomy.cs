            ...
            try {
                ...
                if (args.Parameters.Count != 0) {
                    ...
                    try
                    {
                        ...
                        if (!(!group.StartsWith("Sub") && !group.StartsWith("Amethyst") && !group.StartsWith("Topaz") && !group.StartsWith("Sapphire") && !group.StartsWith("Emerald") && !group.StartsWith("Ruby") && !group.StartsWith("Amber") && !group.StartsWith("Diamond") && !(group == "Wood") && !(group == "BorealWood") && !(group == "PalmWood") && !(group == "EbonWood") && !(group == "PearlWood")))
                        {
                            ...
                            switch (group)
                            {
                                case "Wood":
                                    ...
                                    {
                                        group = "BorealWood";
                                        command = new SQLiteCommand("UPDATE Economy SET Points=" + (points - 10) + " WHERE Username='" + args.Player.User.Name + "'", db);
                                        command.ExecuteNonQuery();
                                    }
                                    break;
                                case "BorealWood":
                                    ...
                                    {
                                        group = "PalmWood";
                                        command = new SQLiteCommand("UPDATE Economy SET Points=" + (points - 20) + " WHERE Username='" + args.Player.User.Name + "'", db);
                                        command.ExecuteNonQuery();
                                    }
                                    break;
                                case "PalmWood":
                                    ...
                                    {
                                        group = "EbonWood";
                                        command = new SQLiteCommand("UPDATE Economy SET Points=" + (points - 30) + " WHERE Username='" + args.Player.User.Name + "'", db);
                                        command.ExecuteNonQuery();
                                    }
                                    break;
                                case "EbonWood":
                                    ...
                                    {
                                        group = "PearlWood";
                                        command = new SQLiteCommand("UPDATE Economy SET Points=" + (points - 40) + " WHERE Username='" + args.Player.User.Name + "'", db);
                                        command.ExecuteNonQuery();
                                    }
                                    break;
                                case "PearlWood":
                                    if (points >= 100)
                                    {
                                        group = "AmethystCopper";
                                        command = new SQLiteCommand("UPDATE Economy SET Points=" + (points - 100) + " WHERE Username='" + args.Player.User.Name + "'", db);
                                        command.ExecuteNonQuery();
                                    }
                                    break;
                                ...
                                default:
                                    string newGroup;
                                    if (group.StartsWith("Amethyst"))
                                    {
                                        newGroup = advancePrestige(group.Substring(8, group.Length - 8), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                            group = "TopazCopper";
                                        }
                                        ...
                                        {
                                            group = "Amethyst" + newGroup;
                                        }
                                    }
                                    else if (group.StartsWith("Topaz"))
                                    {
                                        newGroup = advancePrestige(group.Substring(5, group.Length - 5), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                            group = "SapphireCopper";
                                        }
                                        else
                                        {
                                            group = "Topaz" + newGroup;
                                        }
                                    }
                                    else if (group.StartsWith("Sapphire"))
                                    {
                                        newGroup = advancePrestige(group.Substring(8, group.Length - 8), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                        ...
                                        {
                                            group = "Sapphire" + newGroup;
                                        }
                                    }
                                    else if (group.StartsWith("Emerald"))
                                    {
                                        newGroup = advancePrestige(group.Substring(7, group.Length - 7), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                            group = "RubyCopper";
                                        }
                                        ...
                                        {
                                            group = "Emerald" + newGroup;
                                        }
                                    }
                                    else if (group.StartsWith("Ruby"))
                                    {
                                        newGroup = advancePrestige(group.Substring(4, group.Length - 4), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                            group = "AmberCopper";
                                        }
                                    ...
                                    else if (group.StartsWith("Amber"))
                                    {
                                        newGroup = advancePrestige(group.Substring(5, group.Length - 5), points, args.Player.User.Name);
                                        if (newGroup == "NEWMAIN")
                                        {
                                        ...
                                        {
                                            group = "Amber" + newGroup;
                                        }
                                    }
                                    else if (group.StartsWith("Diamond"))
                                    {
                                        if (group != "DiamondLuminite")
                                        {
                                            newGroup = advancePrestige(group.Substring(7, group.Length - 7), points, args.Player.User.Name);
                                            group = "Diamond" + newGroup;
                                        }
                                        else
                                        {
                                            args.Player.SendErrorMessage("You have already reached the final rank!");
                                        }
                                    }
                                break;
                            }
                            ...
                            db.Close();
                            GC.Collect();
                            if (group != args.Player.User.Group)
                            {
                                Commands.HandleCommand(TSPlayer.Server, "/user group \"" + args.Player.User.Name + "\" " + group);
                                string username = args.Player.User.Name;
                                Commands.HandleCommand(args.Player, "/logout");
                                Commands.HandleCommand(args.Player, "/login \"" + username + "\" " + password);
                                TSPlayer.All.SendMessage(args.Player.User.Name + " has ranked up and is now a " + group, 255, 128, 0);
                                Commands.HandleCommand(TSPlayer.Server, "/firework \"" + args.Player.User.Name + "\" blue");
                            }
                            else if (args.Player.User.Group != "DiamondLuminite")
                            {
                                args.Player.SendErrorMessage("You don't have enough " + PointName + "!");
                            }
                        }
                        else
                        {
